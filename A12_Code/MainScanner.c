...
viper_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern viper_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(viper_void);
 ...
 * -------------------------------------------------------------
 */
viper_void printScannerError(viper_str fmt, ...);
viper_void displayScanner(BufferPointer ptrBuffer);
viper_int getScannerFilesize(viper_str fname);
...
viper_int mainScanner(viper_int argc, viper_str* argv) {
